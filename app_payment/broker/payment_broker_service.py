...
QUEUE_AUTH_EVENTS = f"payment_queue_{_AUTH_SUFFIX}"  # se mantiene por compatibilidad (nombre histórico)
...
async def fetch_auth_public_key_on_startup(max_attempts: int = 60, base_delay: float = 2.0) -> None:
    """
    Intenta obtener la clave pública de Auth al iniciar Payment.
    
    Por qué es necesario:
        - Si esta réplica de Payment arranca DESPUÉS de que Auth publicó 'auth.running',
    ...
    Estrategia:
        ...
        - No bloquea el arranque de Payment (se ejecuta como task).
    ...
    """
    logger.info("[PAYMENT] 🔑 Iniciando obtención de clave pública de Auth al arranque...")
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            logger.info("[PAYMENT] ✅ Clave pública de Auth obtenida al arranque y guardada en %s", PUBLIC_KEY_PATH)
            return
            
        except Exception as exc:
            logger.warning(
                "[PAYMENT] ⏳ (Startup) Auth no disponible aún. Reintento %s/%s. Motivo: %s",
                attempt, max_attempts, exc
            )
    ...
    logger.error("[PAYMENT] ❌ No se pudo obtener la clave pública de Auth tras %s intentos al arranque", max_attempts)
